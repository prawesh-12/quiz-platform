...
const MIGRATION_LOCK_KEY = 4927014;
const MIGRATIONS_DIR = path.join(path.dirname(fileURLToPath(import.meta.url)), "../migrations");

// The whole run executes in ONE transaction guarded by a transaction-scoped advisory
// lock. That is pooler-safe (the transaction is pinned to a single backend and the lock
// auto-releases on COMMIT/ROLLBACK), unlike a session lock which can leak behind a
// transaction-mode pooler. Postgres DDL is transactional, so the run is also all-or-nothing.
export async function runMigrations() {
  const client = await pool.connect();

  try {
    await client.query("BEGIN");
    // Scoped to this transaction (pooler-safe); migrations may build large indexes.
