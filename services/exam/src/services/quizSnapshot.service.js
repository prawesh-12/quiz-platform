...
// Student-safe quiz snapshot cached in Redis; the durable copy lives in quiz_snapshot,
// fed by the quiz service. Question content is never read from the quiz tables here.

const DEFAULT_SNAPSHOT_TTL_MS = 4 * 60 * 60 * 1000;
const SNAPSHOT_TTL_MS = readPositiveIntegerEnv("QUIZ_SNAPSHOT_TTL_MS", DEFAULT_SNAPSHOT_TTL_MS);
...
async function storeSnapshotRow(quizId, questions) {
  try {
    await pool.query(
      `
      INSERT INTO quiz_snapshot (quiz_id, payload)
      VALUES ($1, $2::jsonb)
...
async function loadSnapshotQuestions(quizId) {
  const result = await pool.query(`SELECT payload FROM quiz_snapshot WHERE quiz_id = $1`, [quizId]);
...
export async function deleteQuizSnapshot(quizId) {
  try {
    await pool.query(`DELETE FROM quiz_snapshot WHERE quiz_id = $1`, [quizId]);
  } catch (error) {
