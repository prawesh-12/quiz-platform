...
function parseNaiveDateTimeAsIST(value) {
  ...
  const utcMillis =
    ...
    IST_OFFSET_MINUTES * SECONDS_PER_MINUTE * MILLIS_PER_SECOND;
...
function formatLocalDateTime(value) {
  ...
  const istDate = new Date(date.getTime() + IST_OFFSET_MINUTES * SECONDS_PER_MINUTE * MILLIS_PER_SECOND);
...
function secondsDiff(target, source) {
  return Math.max(0, Math.ceil((target.getTime() - source.getTime()) / MILLIS_PER_SECOND));
}

export function resolveQuizWindow(quiz, nowInput = new Date()) {
  const now = toDate(nowInput) || new Date();
  const durationMins = toPositiveInt(quiz?.duration_mins, DEFAULT_DURATION_MINS);
  ...
  const derivedEnd = new Date(startAt.getTime() + durationMins * SECONDS_PER_MINUTE * MILLIS_PER_SECOND);
...
export function planActivationWindow({
  ...
  const endAt = parsedEnd || new Date(startAt.getTime() + safeDurationMins * SECONDS_PER_MINUTE * MILLIS_PER_SECOND);
