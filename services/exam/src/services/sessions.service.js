...
function withPercent(score, totalPoints) {
  ...
  return Number(((Number(score) / Number(totalPoints)) * FULL_PERCENT).toFixed(PERCENT_DECIMALS));
}
...
function entryResultPayload(quiz, quizWindow, sessionToken, sanitizedQuestions) {
  return {
