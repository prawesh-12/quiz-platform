...
function buildBreakdown(quizQuestions, gradedAnswers) {
...
async function finalizeSessionSubmissionWithQuestions(dbClient, { sessionId, quizQuestions, submittedAnswers, submissionId = null }) {
  ...
  return {
    score,
    total_points,
    breakdown: buildBreakdown(quizQuestions, gradedAnswers)
  };
}
...
// Finalizes pending sessions one batch per transaction so locks release between batches
// and multiple workers can run in parallel via SKIP LOCKED. Caller must commit the quiz's
// 'ended' status before calling this.
export async function finalizePendingSessionsForQuiz(quizId, { batchSize } = {}) {
  ...
  while (true) {
    const batchCount = await finalizeOneBatch(quizId, quizQuestions, effectiveBatchSize);
