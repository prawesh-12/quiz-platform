...
function inMemoryLimit({ key, max, windowMs, message, res, next }) {
  ...
  if (bucket.length >= max) {
    ...
    res.set("Retry-After", String(Math.ceil(retryAfterMs / MS_PER_SECOND)));
    return res.status(TOO_MANY_REQUESTS).json({ error: message });
  }
...
async function redisLimit({ baseKey, max, windowMs, message, res, next }) {
  ...
  if (count > max) {
    ...
    res.set("Retry-After", String(Math.ceil(Math.max(0, ttl) / MS_PER_SECOND)));
    return res.status(TOO_MANY_REQUESTS).json({ error: message });
  }
  return next();
}
...
export default function rateLimit({
  ...
  return async (req, res, next) => {
    ...
    if (getRedis() && isRedisReady()) {
      try {
        return await redisLimit({ baseKey, max, windowMs, message, res, next });
      } catch (error) {
